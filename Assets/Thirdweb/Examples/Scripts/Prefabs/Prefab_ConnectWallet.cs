...
public class Prefab_ConnectWallet : MonoBehaviour
{
    ...
    private Wallet lastWalletChoice;
    ...
    {
        address = null;
        lastWalletChoice = supportedWallets[0];
    ...
    public async void OnConnected()
    {
        try
        {
            PlayerProfile.NickName = await ThirdwebManager.Instance.SDK.wallet.GetAddress();
            lastWalletChoice = wallet;
